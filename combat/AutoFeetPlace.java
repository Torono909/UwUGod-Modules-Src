...
import meow.candycat.uwu.event.events.EventWorldSetBlockState;
...
public class AutoFeetPlace extends Module
{
    ...
    private Setting<Boolean> offGroundToggle;
    ...
    boolean placed;
    @EventHandler
    private Listener<EventWorldSetBlockState> w;
    ...
    public AutoFeetPlace() {
        ...
        this.noglitchblocks = this.register(Settings.b("NoGlitchBlock"));
        this.offGroundToggle = this.register(Settings.b("OffGroundToggle", true));
        boolean y;
        Vec3d[] surroundTargets;
        int length;
        int i = 0;
        Vec3d x;
        this.w = new Listener<EventWorldSetBlockState>(event -> {
            if (this.noglitchblocks.getValue()) {
                y = false;
                surroundTargets = this.surroundTargets;
                length = surroundTargets.length;
                while (i < length) {
                    x = surroundTargets[i];
                    if (x.equals((Object)event.Pos)) {
                        y = true;
                        break;
                    }
                    else {
        ...
        this.surroundTargetsCritical = new Vec3d[] { new Vec3d(0.0, 0.0, 0.0), new Vec3d(1.0, 1.0, 0.0), new Vec3d(0.0, 1.0, 1.0), new Vec3d(-1.0, 1.0, 0.0), new Vec3d(0.0, 1.0, -1.0) };
        ...
        this.blockPerTick = this.register(Settings.d("Blocks per Tick", 4.0));
    ...
    public void onUpdate() {
        this.placed = false;
        if (this.offGroundToggle.getValue() && !AutoFeetPlace.mc.field_71439_g.field_70122_E) {
            this.disable();
            return;
        }
    ...
    public String getHudInfo() {
        return this.placed ? "Placing" : "Idling";
    }
        ...
        for (final EnumFacing side : EnumFacing.values()) {
            ...
            if (!canBeClicked(neighbor)) {
                ...
                if (eyesPos.func_72436_e(hitVec) > 18.0625) {
                    ...
                    final Block blockBelow = AutoFeetPlace.mc.field_71441_e.func_180495_p(neighbor).func_177230_c();
